...
//////////////////////////////
//Lista de Usuarios con los que chateo
$query='SELECT * FROM '.$dbss['prfx']."_chatting WHERE chatto=$bim_id ORDER BY usr_id ASC";
$result=neutral_query($query);

$list="";
$i=neutral_num_rows($result);
for($i; $i > 0; $i-=1){
	$uname_list=neutral_fetch_array($result);
	$list .= $uname_list['usr_id'].'<,>'.$uname_list['usr_name'].'|;|';
}
$uname_list = $list;

//////////////////////////////////////////////////

$query='SELECT usr_name FROM '.$dbss['prfx']."_chatting WHERE usr_id=$u AND chatto=$bim_id";
$result=neutral_query($query);
...
print $uname.'|:|'.$lid.'|:|'.$c.'|:|'.$messages.'|:|'.$uname_list;
?>
